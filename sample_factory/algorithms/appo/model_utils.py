...
def normalize_obs(obs_dict, cfg,):
    if 'nonorm255' in cfg.experiment or 'motif_msghist20_filter_newnormobs_v2_twohidblstats_erew0.0_llmrew0.1_nle_all_but_engrave_mila_seed1' in cfg.experiment:
        mask = None
        if mask is None:
            mask = torch.ones((obs_dict['obs'].shape[0]), dtype=torch.bool).to('cuda')
        ...
        with torch.no_grad():
            ...
            if obs_dict['obs'].dtype != torch.float:
                obs_dict['obs'][mask] = obs_dict['obs'].float()[mask]

            if abs(mean) > EPS:
                obs_dict['obs'][mask].sub_(mean)

            if abs(scale - 1.0) > EPS:
                obs_dict['obs'][mask].mul_(1.0 / scale)
    else:
        with torch.no_grad():
            mean = cfg.obs_subtract_mean
            scale = cfg.obs_scale
...
class ConvEncoder(EncoderBase):
    class ConvEncoderImpl(nn.Module):
        ...
        fusion).
        """
        def __init__(self, activation, conv_filters, fc_layer_size, encoder_extra_fc_layers, obs_shape, obs_key='obs'):
            ...
            self.conv_head_out_size = calc_num_elements(self.conv_head, obs_shape[obs_key])
    ...
    def __init__(self, cfg, obs_space, timing, encoder_subtype, obs_key='obs'):
        super().__init__(cfg, timing)
        obs_shape = get_obs_shape(obs_space)
        input_ch = obs_shape[obs_key][0]
        self.obs_key = obs_key
        log.debug('Num input channels: %d', input_ch)

        # inp_ch, out_ch, filter_size, stride
        if encoder_subtype == 'convnet_simple':
            conv_filters = [[input_ch, 32, 8, 4], [32, 64, 4, 2], [64, 128, 3, 2]]
        elif encoder_subtype == 'convnet_impala':
            conv_filters = [[input_ch, 16, 8, 4], [16, 32, 4, 2]]
        elif encoder_subtype == 'minigrid_convnet_tiny':
            conv_filters = [[input_ch, 16, 3, 1], [16, 32, 2, 1], [32, 64, 2, 1]]
        elif encoder_subtype == 'tty_convnet':
            conv_filters = [[input_ch, 32, 3, 2], [32, 64, 3, 2], [64, 128, 2, 1]]
        else:
            raise NotImplementedError(f'Unknown encoder {encoder_subtype}')

        activation = nonlinearity(self.cfg)
        fc_layer_size = fc_after_encoder_size(self.cfg)
        encoder_extra_fc_layers = self.cfg.encoder_extra_fc_layers

        enc = self.ConvEncoderImpl(activation, conv_filters, fc_layer_size, encoder_extra_fc_layers, obs_shape, obs_key)
        self.enc = torch.jit.script(enc)

        self.encoder_out_size = calc_num_elements(self.enc, obs_shape[obs_key])
...
class ResnetEncoder(EncoderBase):
    def __init__(self, cfg, obs_space, timing, obs_key='obs'):
        super().__init__(cfg, timing)
...
def create_encoder(cfg, 
    ...
    if encoder_custom:
        ...
        encoder = encoder_cls(cfg, obs_space, timing, reward_model)
    else:
...
def create_standard_encoder(cfg, obs_space, timing, encoder_subtype, obs_key='obs'):
    if cfg.encoder_type == 'conv':
        encoder = ConvEncoder(cfg, obs_space, timing, encoder_subtype, obs_key=obs_key)
    # TODO: add encoder_subtype for the rest
    elif cfg.encoder_type == 'resnet':
        encoder = ResnetEncoder(cfg, obs_space, timing, obs_key=obs_key)
    elif cfg.encoder_type == 'mlp':
        encoder = MlpEncoder(cfg, obs_space, timing, obs_key=obs_key)
    else:
...
class PolicyCoreFeedForward(PolicyCoreBase):
    ...
    def __init__(self, cfg, input_size):
        super().__init__(cfg)
        self.cfg = cfg
        self.core_output_size = cfg.hidden_size if cfg.use_mlp_core else input_size
        self.use_mlp_core = cfg.use_mlp_core

        if self.use_mlp_core:
