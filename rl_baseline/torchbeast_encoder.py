...
class TorchBeastEncoder(EncoderBase):
    def __init__(self, cfg, obs_space, timing, reward_model):
        ...
        self.bl_stats_key = 'norm_blstats'
        ...
        self.use_diffstats = cfg.use_diffstats
        ...
        if self.use_diffstats:
            self.embed_diffstats = nn.Sequential(
                nn.Linear(cfg.diffstats_size, self.k_dim),
                nn.ReLU(),
                nn.Linear(self.k_dim, self.k_dim),
                nn.ReLU(),
            )
    ...
    def forward(self, obs_dict):
        ...
        if self.use_diffstats:
            diffstats = obs_dict['diffstats'].float()
            diffstats_emb = self.embed_diffstats(diffstats)
            reps.append(diffstats_emb)
